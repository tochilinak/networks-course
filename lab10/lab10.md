...
![image](https://github.com/tochilinak/networks-course/assets/35286460/606ac007-f025-4915-aa8d-7759a68dc68a)
...
![image](https://github.com/tochilinak/networks-course/assets/35286460/bad600ac-4f66-4830-bf6a-d8feb705b048)
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не всегда меняется TTL (раз в несколько пакетов)
      - Total length, Protocol, Source address и Destination address
      - Время от времени меняется TTL. Всегда меняется Identification. Так как меняется хоть что-то, меняется и Checksum.
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Выглядит как рандом (может зависимость есть в каком-то другом режиме или на другой ОС)

6. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - 0xd930
   - 1

![image](https://github.com/tochilinak/networks-course/assets/35286460/477c087b-b07b-4133-a2a2-1c8c4a294a1a)


7. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification меняется, TTL нет

![image](https://github.com/tochilinak/networks-course/assets/35286460/8b3d8657-3327-4e2f-b065-ac5353605eae)


8. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
...
![image](https://github.com/tochilinak/networks-course/assets/35286460/d1294a5f-540b-4de3-b49d-8144cf2821c1)


9. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
  ...
  *Для этого задания я добавила флаг -I, который заставляет на Unix все работать по ICMP вместо UPD*.

![image](https://github.com/tochilinak/networks-course/assets/35286460/0a45bbb9-1694-404f-ab25-e7b6e8ad2db7)


   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      Если да, то сколько фрагментов было создано?
      - Да, оно было разбито на 3 фрагмента.
   
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Identification остается постоянным, меняется Fragment offset.
